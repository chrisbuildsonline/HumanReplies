...
class PopupManager {
  ...
  async checkAuthStatus() {
    try {
      ...
      const result = await new Promise((resolve) => {
        chrome.storage.sync.get(['userToken', 'userProfile', 'refreshToken', 'tokenExpiry'], resolve);
      });
      ...
      if (result.userToken && result.userProfile) {
        ...
        if (now >= expiry && result.refreshToken) {
          try {
            ...
            const refreshResult = await this.supabase.refreshToken(result.refreshToken);
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      if (tabs[0]) {
        ...
        if (isSupported) {
          ...
          statusElement.textContent = 'Extension Offline (Website not supported)';
  ...
  async handleSignup() {
    try {
      // Show loading state
      const signupBtn = document.querySelector('.login-button[onclick="handleSignup()"]');
  ...
  async clearAuthData() {
    ...
    await new Promise((resolve) => {
      chrome.storage.sync.remove(['userToken', 'userProfile', 'refreshToken', 'tokenExpiry'], resolve);
    });
